...
| [working-activity](https://github.com/ccch1mneyyy/working-activity) | Lively Working-line extension for pi CLI and DSH (Privacy risk: 访问第三方网络地址) (Security note: 存在疑似混淆内容) | `dsh plugin --profile web add github:ccch1mneyyy/working-activity` |
...
| [Awesome-DeepSeek-Harness-Plugins](https://github.com/Zhiyuan-Fan/Awesome-DeepSeek-Harness-Plugins) | English \| 简体中文 (Privacy risk: 访问第三方网络地址) | `dsh plugin --profile web add github:Zhiyuan-Fan/Awesome-DeepSeek-Harness-Plugins` |
...
| [DSH-Transparent-UI-Plugin](https://github.com/WYH66666666/DSH-Transparent-UI-Plugin) | English \| 中文 (Privacy risk: 访问第三方网络地址) (Security note: 存在疑似混淆内容) | `dsh plugin --profile web add github:WYH66666666/DSH-Transparent-UI-Plugin` |
...
| [deepseek-harness-desktop](https://github.com/ningbainb/deepseek-harness-desktop) | 中文 \| English (Privacy risk: 读取本地凭据文件（如 .ssh/.aws/.npmrc）; 访问第三方网络地址; 读取凭据类环境变量并发送到网络，可能泄露密钥) | `dsh plugin --profile web add github:ningbainb/deepseek-harness-desktop` |
...
| [dsh-web-plugin-manager](https://github.com/LX2000WASD/dsh-web-plugin-manager) | 中文 \| English (Privacy risk: 访问第三方网络地址; 读取本地凭据文件（如 .ssh/.aws/.npmrc）; 读取凭据类环境变量并发送到网络，可能泄露密钥) (Security note: 存在疑似混淆内容; 使用动态代码或子进程执行) | `dsh plugin --profile web add github:LX2000WASD/dsh-web-plugin-manager` |
...
| [deepseek-harness-studio](https://github.com/fufankeji/deepseek-harness-studio) | > Status key: ✅ Available; 🗓️ Planned. The desktop development workspace, public Plugin Center, and Chinese DeepSeek controls are available today. Planned capabilities will be updated only after the corresponding workflow is runnable. (Privacy risk: 访问第三方网络地址) | `dsh plugin --profile web add github:fufankeji/deepseek-harness-studio` |
...
| [awesome-dsh-plugin](https://github.com/beancookie/awesome-dsh-plugin) | English \| 中文 (Privacy risk: 读取浏览器 Cookie/存储并发送到网络; 访问第三方网络地址) | `dsh plugin --profile web add github:beancookie/awesome-dsh-plugin` |
...
| [dsh-find-plugin](https://github.com/awesome-dsh-plugin/dsh-find-plugin) | English \| 中文 (Privacy risk: 访问第三方网络地址) (Security note: 存在疑似混淆内容) | `dsh plugin --profile web add github:awesome-dsh-plugin/dsh-find-plugin` |
| [dsh-data-agent](https://github.com/omdsh-dev/dsh-data-agent) | 中文 \| English (Privacy risk: 访问第三方网络地址; 读取浏览器 Cookie/存储并发送到网络; 读取环境变量（可能包含敏感信息）) | `dsh plugin --profile web add github:omdsh-dev/dsh-data-agent` |
...
| [dsh-undo-plugin](https://github.com/lire1131/dsh-undo-plugin) | > English \| 中文 \| Changelog (Privacy risk: 访问第三方网络地址; 读取浏览器 Cookie/存储并发送到网络) | `dsh plugin --profile web add github:lire1131/dsh-undo-plugin` |
...
| [dsh-lark](https://github.com/omdsh-dev/dsh-lark) | English \| 简体中文 (Privacy risk: 访问第三方网络地址) (Security note: 存在疑似混淆内容; 使用动态代码或子进程执行) | `dsh plugin --profile web add github:omdsh-dev/dsh-lark` |
...
| [dsh-plugin-cc](https://github.com/cpj-dev/dsh-plugin-cc) | English \| 简体中文 (Privacy risk: 访问第三方网络地址; 读取环境变量（可能包含敏感信息）) | `dsh plugin --profile web add github:cpj-dev/dsh-plugin-cc` |
...
| [dsh-plugin-marketplace](https://github.com/AwesomeHou/dsh-plugin-marketplace) | English \| 中文 (Privacy risk: 访问第三方网络地址; 读取凭据类环境变量并发送到网络，可能泄露密钥) (Security note: 使用动态代码或子进程执行) | `dsh plugin --profile web add github:AwesomeHou/dsh-plugin-marketplace` |
| [dshcode](https://github.com/whitelonng/dshcode) | English \| 中文 (Privacy risk: 访问第三方网络地址) | `dsh plugin --profile web add github:whitelonng/dshcode` |
| [dsh-plugin-template](https://github.com/bugmaker2/dsh-plugin-template) | English \| 中文 (Security note: 存在疑似混淆内容) | `dsh plugin --profile web add github:bugmaker2/dsh-plugin-template` |
...
| [dsh-share](https://github.com/hellodigua/dsh-share) | 简体中文 \| English (Privacy risk: 访问第三方网络地址; 读取浏览器 Cookie/存储并发送到网络; 读取环境变量并访问第三方地址，需确认未外发敏感信息) (Security note: 使用动态代码或子进程执行; 存在 Base64 解码行为; 存在疑似混淆内容) | `dsh plugin --profile web add github:hellodigua/dsh-share` |
