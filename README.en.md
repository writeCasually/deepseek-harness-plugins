...
| [dsh-vision-toolkit](https://github.com/Anionex/dsh-vision-toolkit) | A more powerful vision toolkit—give text-only models in DeepSeek Harness eyes: image Q&A, long-screenshot OCR, UI restoration, and GUI visual tasks in one toolkit and Skill. | `dsh plugin --profile web add github:Anionex/dsh-vision-toolkit` |
...
| [dsh-agent-teams](https://github.com/NanmiCoder/dsh-agent-teams) | dsh-agent-teams turns the current DeepSeek Harness session into a captain that can assemble durable sub-agents, split a goal into dependency-aware tasks, and coordinate work through direct messages. | `dsh plugin --profile web add github:NanmiCoder/dsh-agent-teams` |
...
| [oh-dsh](https://github.com/hust-open-atom-club/oh-dsh) | 🖥️ Three interaction surfaces Use the same ohdsh command to start Desktop, Web, or TUI. All surfaces share sessions, credentials, skins, and plugin caches while keeping separate Profiles. | `Use the desktop/Web/TUI install scripts in the README to install the distribution` |
...
| [dsh-visualize](https://github.com/Nagi-ovo/dsh-visualize) | DSH does not have to answer with text alone. When the model calls visualize, the Web UI renders an interactive card inside the conversation for simulators, charts, comparison panels, and UI mockups. | `dsh plugin --profile web add github:Nagi-ovo/dsh-visualize` |
...
| [dsh-genui](https://github.com/omdsh-dev/dsh-genui) | > Give the model's answers a face — the text is still there, and an interactive UI is already live. | `dsh plugin --profile web add github:omdsh-dev/dsh-genui` |
...
| [anime-find](https://github.com/cocofhu/anime-find) | DeepSeek Harness 搜番插件：对话内多源搜索番剧，卡片展示 Bangumi 评分与详情，支持复制磁力。 | `dsh plugin --profile web add github:cocofhu/anime-find` |
...
| [dsh-turn-rewind](https://github.com/Anionex/dsh-turn-rewind) | DSH Turn Rewind is maintained by anionex. If you would like to follow my future work, follow me on X or GitHub. | `dsh plugin --profile web add github:Anionex/dsh-turn-rewind` |
...
| [SpecFusion](https://github.com/wxkingstar/SpecFusion) | 在 DeepSeek Harness / Claude Code / Cursor / Codex / Gemini CLI 里直接搜索 20 个中国开放平台的 65,600+ 篇 API 文档；零配置，支持 Skill 与 DSH 原生插件。 | `dsh plugin --profile web add github:wxkingstar/SpecFusion` |
...
| [dsh-open-in-vscode](https://github.com/omdsh-dev/dsh-open-in-vscode) | Open a workspace directory in VS Code straight from the DeepSeek Harness web GUI: every real Workspace row in the sidebar gains an Open in VSCode row inside its … overflow menu. | `dsh plugin --profile web add github:omdsh-dev/dsh-open-in-vscode` |
...
| [dsh-deepseek-flow](https://github.com/kanghelyu/dsh-deepseek-flow) | - Markdown as the source of truth — one master WORKFLOW.md, plus one STEP.md workspace for each step. | `dsh plugin --profile web add github:kanghelyu/dsh-deepseek-flow` |
