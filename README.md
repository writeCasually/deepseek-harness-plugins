...
- 网页汇总页：支持搜索、按分类筛选，展示每个插件的名称、功能简介、用法与项目链接。
- 单一数据源：`docs/plugins.json` 同时驱动网页与 README 插件列表。
- 每日自动检索：GitHub Actions 每天检索 `dsh-plugin` 话题并做静态安全审查。
...
| 插件名称 | 功能简介 | 用法 | 项目链接 |
| --- | --- | --- | --- |
| [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) | DeepSeek Harness 核心运行时：Everything is a Plugin。 | `npx @deepseek-ai/dsh 启动核心；插件经 dsh plugin --profile web add <源> 挂载` | [查看](https://github.com/deepseek-ai/deepseek-harness) |
...
共收录 24 个插件，数据来源与更新时间见 [docs/plugins.json](docs/plugins.json)。
...
2. 下载仓库 README 与源码文件树，执行静态安全扫描。
3. 将通过审查的插件写入 `docs/plugins.json`，并重新生成 README 插件列表。
4. 以 Pull Request 形式提交变更，人工合并后发布。
